...
export default function BookingModal({
  ...
  const formBody = (
    <div
      ...
      className={[
        ...
        inline ? "w-full rounded-[1.75rem]" : "relative w-full rounded-2xl",
    ...
    >
      <div className="h-0.75 w-full shrink-0 rounded-t-2xl" style={{ background: `linear-gradient(to right, transparent, ${GOLD}, ${DARK}, ${GOLD}, transparent)` }} />

      <div className="shrink-0 px-6 sm:px-8 pt-7 pb-5 text-center relative overflow-hidden" style={{ background: `linear-gradient(160deg, ${LIGHT} 0%, white 100%)` }}>
        <div className="absolute top-3 right-6 w-20 h-20 rounded-full opacity-[0.06]" style={{ background: `radial-gradient(circle, ${GOLD}, transparent)` }} />
        <div className="absolute bottom-0 left-4 w-14 h-14 rounded-full opacity-[0.05]" style={{ background: `radial-gradient(circle, ${DARK}, transparent)` }} />

        <div className="inline-flex items-center justify-center w-11 h-11 rounded-2xl mb-3 shadow-md" style={{ background: `linear-gradient(135deg, ${GOLD}, ${DARK})` }}>
        ...
        <h2 className="text-2xl sm:text-[1.7rem] font-bold text-stone-800 leading-tight">{title}</h2>
        <Ornament />
        <p className="text-stone-400 text-[13px] mt-2.5 font-sans" style={{ fontFamily: "'DM Sans', sans-serif" }}>{subtitle}</p>

        {!inline && (
          <button onClick={() => onClose?.()} aria-label="Close" className="absolute top-4 right-4 p-2 rounded-xl text-stone-400 hover:text-stone-600 hover:bg-stone-100 transition-all duration-150">
      ...
      <div className="flex-1 px-6 sm:px-8 pt-5 pb-8 overflow-y-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        {submitted ? (
          <div className="flex flex-col items-center justify-center py-8 text-center gap-4">
            <div className="w-16 h-16 rounded-full flex items-center justify-center shadow-xl" style={{ background: `linear-gradient(135deg, ${GOLD}, ${DARK})` }}>
            ...
            <div>
              <h3 className="text-xl font-bold text-stone-800 mb-1" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Inquiry Sent!</h3>
              <p className="text-stone-500 text-sm mb-4">Our travel expert will reach out within 24 hours.</p>
              {(inline || embedded) && (
                <button onClick={() => { setForm(EMPTY); setSubmitted(false); setStep(1); }} className="text-sm font-semibold underline underline-offset-4" style={{ color: GOLD }}>Submit another enquiry</button>
              )}
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} noValidate>
            ...
            {errors.submit && <p className="bg-red-50 text-red-600 p-3 rounded-lg text-xs mb-4 text-center">{errors.submit}</p>}

            {/* Step 1 */}
            <div className="space-y-4 transition-all duration-200" style={{ opacity: animating ? 0 : 1, transform: animating ? "translateX(-12px)" : "none", display: step === 1 ? "block" : "none" }}>
              <Field label="Full Name" icon="👤" error={errors.name}>
                <input type="text" name="name" placeholder="e.g. Ravi Sharma" value={form.name} onChange={change} className={inputCls} />
              </Field>
              ...
              <Field label="Phone Number" icon="📞" error={errors.phone}>
                <input type="tel" name="phone" placeholder="+91 98765 43210" value={form.phone} onChange={change} className={inputCls} />
              </Field>
              <GradBtn type="button" onClick={goStep2} className="w-full py-4 rounded-xl text-white font-semibold text-[15px] tracking-wide mt-2">
                Continue
            ...
            {/* Step 2 */}
            <div className="space-y-4 transition-all duration-200" style={{ opacity: animating ? 0 : 1, transform: animating ? "translateX(12px)" : "none", display: step === 2 ? "block" : "none" }}>
              <Field label="Destination" icon="📍" error={errors.place}>
                <input type="text" name="place" placeholder="e.g. Kedarnath..." value={form.place} onChange={change} className={inputCls} />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Travel Date" icon="📅" error={errors.date}>
                  <input type="date" name="date" value={form.date} onChange={change} className={inputCls + " text-stone-600"} min={new Date().toISOString().split("T")[0]} />
                </Field>
              ...
              <Field label="Special Requests (optional)" icon="💬">
                <textarea name="message" rows="3" placeholder="Special requirements..." value={form.message} onChange={change} className={inputCls + " resize-none"} />
              </Field>

              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setStep(1)} className="flex-1 py-4 rounded-xl border-2 border-stone-200 text-stone-600 font-semibold text-sm hover:bg-stone-50 active:scale-[0.98] transition-all">← Back</button>
                <GradBtn type="submit" loading={loading} className="flex-2 py-4 rounded-xl text-white font-semibold text-[15px] tracking-wide shadow-lg">
                  {submitLabel}
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </GradBtn>
              </div>
  ...
  const assets = (
    <>
      ...
      <style>{`
        ...
        @keyframes bmSlideUp { from { opacity: 0; transform: translateY(28px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
  ...
  if (inline || embedded) return <>{assets}{formBody}</>;
